...
helm upgrade  --install ${RELEASE_NAME} ./chart/${CHART_NAME} --set image.repository=${IMAGE_REPOSITORY},image.tag=${BUILD_NUMBER},secretKeyRefs.conversation=${CONVERSATION} --namespace ${CLUSTER_NAMESPACE}
